...
@Composable
fun EditAccountClientScreen(viewModel: EditAccountClientViewModel = hiltViewModel()) {
    ...
    AccountsContent(
        ...
        editClient = viewModel::editClient
    )
    LaunchedEffect(Unit) {
        viewModel.effect.collectLatest { effect ->
            when (effect) {
                is EditAccountClientEffect.ShowToastError -> {
                    Toast.makeText(context, effect.message, Toast.LENGTH_SHORT).show()
                }

                is EditAccountClientEffect.SucceedEditClient -> {
    ...
    Column(
        ...
        if (state.isLoading) {
            ...
            Column(
                ...
                PositiveButton(
                    text = "تعديل العميل",
                    isLoading = state.isLoadingEditClient,
                    isEnabled = state.visibilityEditClientButton,
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(48.dp)
                ) {
