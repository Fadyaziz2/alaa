...
class BuysViewModel @Inject constructor(
    ...
    BaseViewModel<BuysUiState, BuysUiEffect>(BuysUiState()) {
    ...
    fun onClickDeleteItemInPreviousInvoice(currentItem: Item) {
        ...
        tryToExecute(function = {
            ...
            getAllInvoices()
            sendNewEffect(BuysUiEffect.ShowToastError(it))
            updateState {
                it.copy(
                    isLoading = false,
    ...
    fun onClickSaveNewInvoice() {
        ...
        tryToExecute(function = {
            ...
            sendNewEffect(BuysUiEffect.ShowToastError(messege))
    ...
    private fun getAllInvoices() {
        ...
        tryToExecute(function = { invoiceUseCase.getAllInvoices() }, onSuccess = { cusResInvoices ->
            val invoices = cusResInvoices.resourcesInvoices
            updateState {
                it.copy(
                    isLoading = false,
    ...
    fun navigateBackToDetails() {
        if (state.value.addNewInvoiceState.visible) {
            if (state.value.addNewInvoiceState.addNewItemVisible)
                updateState {
                    it.copy(
                ...
                updateState { it.copy(addNewInvoiceState = AddNewInvoice()) }
        } else if (state.value.searchInvoice.visible) {
            if (state.value.searchInvoice.selectedInvoice == null)
                updateState {
                    it.copy(
                        searchInvoice = it.searchInvoice.copy(
                            visible = false
        ...
        } else sendNewEffect(BuysUiEffect.NavigateBack)
    }
    ...
    fun onClickDone() {
        ...
        tryToExecute(function = { }, onSuccess = {
            ...
            sendNewEffect(BuysUiEffect.ShowToastSuccessAddingNewItemToNewInvoice)
    ...
    private fun onError(error: ErrorUiState, message: String) {
        ...
        sendNewEffect(BuysUiEffect.ShowToastError(message))
    }
    ...
    fun searchProductUsingId(id: Long) {
        ...
        if (selectedProduct == null) {
            sendNewEffect(BuysUiEffect.ShowToastError("لا يوجد منتج بهذا الكود"))
            return
        }
        updateState {
            it.copy(
